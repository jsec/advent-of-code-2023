import {
  Position, hashPosition, manhattan, unhashPosition
} from './util/grid';
...
const findIntersections = (wires: string[][]): Position[] => {
  ...
  return wires[0]!
    .filter(v => sets
      .slice(1)
      .every(w => w.has(v))).map(unhashPosition);
};
...
const p2 = (wires: string[][]): number => {
  const sums = findIntersections(wires).map(p => wires.map(w => w.indexOf(hashPosition(p)) + 1)
    .reduce((a, c) => a + c, 0));
...
const wires = getInputLines()
  .map(line => line
    ...
    .map((string_) => {
      return {
        direction: string_[0]!,
        length: Number.parseInt(string_.slice(1)),
      };
    }))
  .map(generatePath);
